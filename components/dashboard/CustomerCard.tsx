...
export function CustomerCard({
  ...
  const elapsedSeconds = now
    ? Math.max(
        0,
        Math.floor(
          (now.getTime() - new Date(checkIn.created_at).getTime()) / 1000,
        ),
      )
    : 0
  const needsAttention =
    !isClosed && elapsedSeconds >= ATTENTION_THRESHOLD_SECONDS
    ...
    >
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-lg font-medium text-[#6f7f86]">
        ...
        <div className="flex items-center gap-2 rounded-full bg-white px-3 py-1 text-sm font-bold tabular-nums text-muted-foreground shadow-sm">
          ...
          {isClosed
            ? formatClosedLabel(checkIn, now)
            : formatWaitingLabel(elapsedSeconds)}
        </div>
      </div>
...
function formatClosedLabel(checkIn: CheckIn, now: Date | null) {
  ...
  const elapsedSeconds = now
    ? Math.max(
        0,
        Math.floor(
          (now.getTime() - new Date(checkIn.closed_at).getTime()) / 1000,
        ),
      )
    : 0
...
function formatServiceLabel(checkIn: CheckIn): ReactNode {
  ...
  if (
    checkIn.service_type === 'windshield' &&
    checkIn.windshield_intent === 'inspection'
  ) {
  ...
  if (
    checkIn.service_type === 'rock_chip' &&
    checkIn.payment_type === 'insurance'
  ) {
