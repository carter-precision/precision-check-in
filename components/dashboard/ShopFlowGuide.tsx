...
const SHOP_FLOWS: ShopFlow[] = [
  {
    ...
    summary:
      'Inspect, create the job, complete the repair, and collect payment.',
    steps: [
      {
        roles: ['Tech'],
        action:
          'Inspect the chip and determine whether it needs a repair or replacement.',
      },
      {
        roles: ['Customer', 'Tech'],
        action:
          'Return to the kiosk with the customer and have them complete the rock chip quote. This creates the job and adds it to the schedule.',
      },
      {
        roles: ['Tech'],
      ...
      },
      {
        roles: ['Tech'],
        action:
          'Return the vehicle, check out the customer, and collect payment.',
      },
    ],
  },
  {
    ...
    summary:
      'Guide the customer through filing and verifying their own insurance claim.',
    steps: [
      {
        roles: ['Tech'],
        action:
      ...
      },
      {
        roles: ['Customer', 'Tech'],
        action:
          'Direct the customer to file their claim with insurance and give them the office number: 801-520-3131',
      },
      {
        roles: ['Customer'],
        action:
          'Contact the insurance group by phone, mobile app, or website. File the claim, then call the office to verify a same-day appointment.',
        warning:
          'The customer must set up and confirm the claim—the claim is not staff-initiated.',
      },
      {
        roles: ['Customer', 'Tech'],
        action:
          'Explain that after the claim is completed and verified with the office, the customer can check in again as an appointment and the team will complete the work.',
      },
    ],
  },
  {
    ...
    label: 'Quote Inquiry',
    summary:
      'The customer submits a kiosk form and the office handles follow-up.',
    steps: [
      {
        roles: ['Customer'],
        action: 'Fill out the quote form at the kiosk.',
        warning:
          'The office follows up by phone to book the job. Nothing else is needed from the on-site team.',
      },
    ],
  },
  {
    ...
    summary:
      'Resolve on-site when possible, consult Quality Control, and hand off cleanly.',
    steps: [
      {
        roles: ['Manager'],
      ...
      },
      {
        roles: ['Manager'],
        action:
          'Troubleshoot the fix and communicate with the customer. Verify with a supervisor if the cost to correct the issue exceeds $300.',
        branch: {
          roles: ['Manager', 'Quality Control'],
          action:
            'If you are unsure what to do, consult with Quality Control and/or hand the job off to them for the fix.',
        },
      },
      {
        roles: ['Manager'],
        action:
          'Escalate to Purchasing for scheduling with the customer. Make sure the handoff is clean and thorough.',
      },
    ],
  },
  {
    ...
    label: 'Appointment',
    summary:
      'Inspect, set expectations, communicate delays, and complete Dispatch notes.',
    steps: [
      {
        roles: ['Tech'],
        action:
          'Handle the customer directly; complete the inspection at the vehicle.',
      },
      {
        roles: ['Tech'],
        action:
          'Confirm whether the customer is waiting on-site or leaving the vehicle.',
      },
      {
        roles: ['Tech'],
        action:
          'Check in with the customer right before the windshield is set and give them an ETA.',
      },
      {
        roles: ['Tech'],
        action:
          'If the installation runs long, proactively update the customer.',
        warning:
          'Non-negotiable: never let a customer wait 45+ minutes without an update.',
      },
      {
        roles: ['Tech'],
        action:
          'Communicate with the customer and fill out the Dispatch notes.',
      },
    ],
  },
]
...
export function ShopFlowGuide({
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        className="flex h-[min(760px,calc(100vh-2rem))] sm:max-w-6xl flex-col gap-0 overflow-hidden rounded-[2rem] border border-[#d7e1e3] p-0 text-[#1f2933] shadow-2xl"
        showCloseButton
      >
        ...
        {flow.steps.map((step, index) => (
          ...
          >
            {index < flow.steps.length - 1 && (
              <div
                className="absolute left-5 top-10 h-[calc(100%+0.75rem)] w-px bg-[#cbd8db]"
                aria-hidden="true"
              />
            )}
...
function RoleBadge({ role }: { role: FlowRole }) {
  return (
    <span
      className={`rounded-full px-3 py-1 text-xs font-black uppercase tracking-[0.06em] ${ROLE_STYLES[role]}`}
    >
      {role}
    </span>
  )
