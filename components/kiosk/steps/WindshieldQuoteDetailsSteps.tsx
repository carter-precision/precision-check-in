...
export function WindshieldInsuranceDetailsStep({
  data,
  updateData,
  goTo,
}: KioskStepProps) {
      ...
      <QuoteForm>
        ...
        <QuoteField id="insurance-carrier" label="Insurance carrier">
          <QuoteInput
            ...
            onChange={(event) =>
              updateData({ insuranceCarrier: event.target.value })
            }
          />
        ...
        <QuoteField id="policy-holder-name" label="Policy holder name">
          <QuoteInput
            ...
            onChange={(event) =>
              updateData({ policyHolderName: event.target.value })
            }
          />
        </QuoteField>
        ...
        <QuoteField id="policy-holder-phone" label="Policy holder phone">
          <QuoteInput
            ...
            onChange={(event) =>
              updateData({ policyHolderPhone: event.target.value })
            }
          />
        </QuoteField>
        ...
        <QuoteField label="Do you know your deductible?" optional>
          <div className="grid grid-cols-2 gap-3">
            ...
            <QuoteToggle
              ...
              onClick={() =>
                updateData({ knowsDeductible: false, deductibleAmount: '' })
              }
            >
              Not sure
            </QuoteToggle>
        ...
        {data.knowsDeductible === true && (
          <QuoteField id="deductible" label="Deductible amount">
            <QuoteInput
              ...
              placeholder="$250"
              onChange={(event) =>
                updateData({ deductibleAmount: event.target.value })
              }
            />
          </QuoteField>
        )}
        ...
        {data.knowsDeductible === false && (
          <p className="rounded-xl bg-accent-tint p-4 text-base font-medium text-[#40525a]">
            No problem. We'll verify your coverage and deductible before any
            work is scheduled.
          </p>
        )}
...
export function WindshieldVehicleStep({
  ...
  const hasVehicle = data.vinUnknown
    ? Boolean(
        data.vehicleYear && data.vehicleMake.trim() && data.vehicleModel.trim(),
      )
    ...
    return (
      <QuoteField id="vehicle-vin" label="VIN" optional={optional}>
        <QuoteInput
          ...
          onChange={(event) =>
            updateData({ vin: event.target.value.toUpperCase() })
          }
        />
        <QuoteHelperButton
          onClick={() => updateData({ vinUnknown: true, vin: '' })}
        >
          I don't have my VIN
        </QuoteHelperButton>
      </QuoteField>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <QuoteField id="vehicle-make" label="Vehicle make" optional={optional}>
          <QuoteInput
            ...
            placeholder="Toyota"
            onChange={(event) =>
              updateData({ vehicleMake: event.target.value })
            }
          />
        </QuoteField>
        <QuoteField
        ...
        >
          <QuoteInput
            ...
            placeholder="Camry"
            onChange={(event) =>
              updateData({ vehicleModel: event.target.value })
            }
          />
        </QuoteField>
      </div>
