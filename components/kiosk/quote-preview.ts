...
export function getQuotePreview(data: KioskData): QuotePreview {
  ...
  if (['driver_front', 'passenger_front'].includes(data.glassType ?? ''))
    [low, high] = [190, 260]
  if (['driver_rear', 'passenger_rear'].includes(data.glassType ?? ''))
    [low, high] = [170, 230]
  if (['driver_quarter', 'passenger_quarter'].includes(data.glassType ?? ''))
    [low, high] = [140, 195]
  if (data.glassType === 'sunroof') [low, high] = [380, 560]

  const extras = data.vehicleFeatures.filter(
