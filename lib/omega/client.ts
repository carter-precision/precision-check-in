...
export async function getOmegaInvoice(
  invoiceId: string,
): Promise<OmegaInvoice> {
  const payload = await omegaRequest(
  ...
  if (!parsed.success) {
    throw new OmegaApiError(
      'Omega returned an invalid invoice',
      'invalid_response',
    )
  }

  const customerName = joinValues(
    parsed.data.customer_fname,
    parsed.data.customer_surname,
  )
  ...
  if (!customerName) {
    throw new OmegaApiError(
      'Omega invoice is missing a customer name',
      'invalid_response',
    )
  }
...
async function omegaRequest(path: string) {
  ...
  if (!response.ok) {
    throw new OmegaApiError(
      `Omega request failed with status ${response.status}`,
      'unavailable',
    )
  }
...
function parseOmegaTimestamp(value: string | number) {
  ...
  if (!Number.isFinite(parsed)) {
    throw new OmegaApiError(
      'Omega returned an invalid appointment time',
      'invalid_response',
    )
  }
