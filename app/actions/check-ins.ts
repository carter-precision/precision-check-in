...
const locationSlugSchema = z.string().trim().toLowerCase().regex(/^[a-z0-9-]+$/)
...
const kioskCheckInSchema = z
    .object({
        ...
        serviceType: z.enum(["windshield", "rock_chip", "other", "bell"]).nullable().optional(),
...
export async function closeCheckInAction(id: string, locationSlug: string) {
    ...
    const locationId = await requireDeviceForLocation("dashboard", parsed.locationSlug)
